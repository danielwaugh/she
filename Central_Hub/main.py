...
URL2 = "http://72.199.201.49:8081/update_hub.php"
info = {'id': '1'}

appstatuses = ["OFF", "OFF", "OFF", "OFF", "OFF", "OFF"]
...
while(1):
	...
	if(flag):
		for row in Remote:
			data_string = data_string + str(row) + "_"
		flag = 0
	try:		
		data_string = data_string[:-1]
		data_send = {'data': data_string}
		requests.post(URL2, data = data_send)
	except:
		data_send = ""

	try:	
		r = requests.post(URL, data = info)
	        statuses = r.text.split()
	        j = 0
	        for toggle in statuses:
			if (appstatuses[j] != toggle):
				appstatuses[j] = toggle
				if(toggle == "ON"):
					RemoteNew[j] = 1
				if(toggle == "OFF"):
					RemoteNew[j] = 0
